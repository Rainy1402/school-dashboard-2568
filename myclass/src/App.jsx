...
function createClient(baseUrl, apiKey) {
  ...
  function qb(table) {
    ...
    const q = {
      select(fields="*", opts={}) { _sel=fields.replace(/\s+/g,""); if(opts.count)_countMode=opts.count; if(opts.head)_isHead=opts.head; return q },
      ...
      async then(resolve, reject) {
        try {
          ...
          const res=await fetch(ep,{method:_isHead?"HEAD":"GET",headers:rh})
          if(_isHead){ const rng=res.headers.get("Content-Range")||"*/0"; resolve({count:parseInt(rng.split("/")[1]||"0")||0,data:null,error:null}); return }
          const body=await res.json()
          if(!res.ok){resolve({data:null,error:body,count:null});return}
          let count=null
          if(_countMode==="exact"){const rng=res.headers.get("Content-Range")||""; if(rng.includes("/"))count=parseInt(rng.split("/")[1])||0}
          if(_single) resolve({data:Array.isArray(body)?(body[0]??null):(body??null),error:null})
          else resolve({data:Array.isArray(body)?body:[body],error:null,count})
        } catch(e){reject(e)}
      }
    }
    return q
  ...
  return {
    from(table) {
      return {
        ...
        async insert(data) {
          try {
            const res=await fetch(`${url}/rest/v1/${table}`,{method:"POST",headers:{...h,Prefer:"return=minimal"},body:JSON.stringify(data)})
            if(res.status===201||res.status===204) return {data:null,error:null}
            const body=await res.json(); return res.ok?{data:body,error:null}:{data:null,error:body}
          } catch(e){return{data:null,error:{message:e.message}}}
        },
        update(data) {
          const _f=[]
          const u = { eq(col,val){_f.push(`${col}=eq.${encodeURIComponent(val)}`);return u},
            async then(resolve){
              try{const res=await fetch(`${url}/rest/v1/${table}?${_f.join("&")}`,{method:"PATCH",headers:{...h,Prefer:"return=minimal"},body:JSON.stringify(data)})
              if(res.status===204||res.status===200){resolve({data:null,error:null});return}
              const body=await res.json(); resolve(res.ok?{data:body,error:null}:{data:null,error:body})}catch(e){resolve({data:null,error:{message:e.message}})}}}
          return u
        },
        delete() {
          const _f=[]
          const d = { eq(col,val){_f.push(`${col}=eq.${encodeURIComponent(val)}`);return d},
            async then(resolve){
              try{const res=await fetch(`${url}/rest/v1/${table}?${_f.join("&")}`,{method:"DELETE",headers:h})
              if(!res.ok){try{const b=await res.json();resolve({error:b})}catch{resolve({error:{message:`HTTP ${res.status}}`}})}}else resolve({error:null})}catch(e){resolve({error:{message:e.message}})}}}
          return d
        }
      }
    }
...
const T={purple:"#7C3AED",purpleL:"#EDE9FE",blue:"#2563EB",blueL:"#DBEAFE",green:"#16A34A",greenL:"#DCFCE7",orange:"#EA580C",orangeL:"#FFEDD5",red:"#DC2626",redL:"#FEE2E2",yellow:"#D97706",yellowL:"#FEF3C7",slate:"#F8FAFC",border:"#E2E8F0",text:"#1E293B",muted:"#94A3B8"}
...
const ss={
  ...
  overlay:{position:"absolute",inset:0,background:"rgba(0,0,0,0.42)",zIndex:60,display:"flex",alignItems:"center",justifyContent:"center",padding:16},
  ...
  modal:{background:"#fff",borderRadius:20,width:500,maxWidth:"100%",maxHeight:"90vh",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 24px 60px rgba(0,0,0,0.22)"},
  mHead:{padding:"16px 20px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"},
...
function Toast({list}){if(!list.length)return null;return <div style={{position:"fixed",top:12,right:12,zIndex:200,display:"flex",flexDirection:"column",gap:8}}>{list.map(t=><div key={t.id} style={{background:t.type==="err"?T.red:T.green,color:"#fff",borderRadius:12,padding:"11px 16px",fontSize:13.5,fontWeight:600,display:"flex",alignItems:"center",gap:8,fontFamily:"inherit",boxShadow:"0 4px 20px rgba(0,0,0,0.18)",maxWidth:310}}>{t.type==="err"?<AlertCircle size={15}/>:<CheckCircle size={15}/>}{t.msg}</div>)}</div>}
...
function useQ(fn,deps=[]){const[data,setData]=useState(null);const[loading,setLoading]=useState(true);const[error,setError]=useState(null);const reload=useCallback(async()=>{setLoading(true);setError(null);try{setData(await fn())}catch(e){setError(e.message)}finally{setLoading(false)}},deps);useEffect(()=>{reload()},[reload]);return{data,loading,error,reload}}
function useDash(sb){return useQ(async()=>{const[sc,cc,sesc,att]=await Promise.all([sb.from("students").select("id",{count:"exact",head:true}),sb.from("classes").select("id",{count:"exact",head:true}),sb.from("teaching_sessions").select("id",{count:"exact",head:true}),sb.from("attendance").select("status")]);const a=att.data||[];return{students:sc.count||0,classes:cc.count||0,sessions:sesc.count||0,att:{present:a.filter(x=>x.status==="present").length,absent:a.filter(x=>x.status==="absent").length,late:a.filter(x=>x.status==="late").length,leave:a.filter(x=>x.status==="leave").length,total:a.length}}},[sb])}
function useRecentSessions(sb){return useQ(async()=>{const{data,error}=await sb.from("teaching_sessions").select("*,classes(class_name,subject_name,color)").order("created_at",{ascending:false}).limit(6);if(error)throw error;return data||[]},[sb])}
function useClasses(sb){return useQ(async()=>{const{data,error}=await sb.from("classes").select("*,class_students(count)").order("created_at",{ascending:false});if(error)throw error;return data||[]},[sb])}
function useStudents(sb,search,level){return useQ(async()=>{let q=sb.from("students").select("*").order("level").order("room").order("student_no");if(level!=="ทั้งหมด")q=q.eq("level",level);if(search)q=q.ilike("full_name",`%${search}%`);const{data,error}=await q;if(error)throw error;return data||[]},[sb,search,level])}
function useAllSessions(sb){return useQ(async()=>{const{data,error}=await sb.from("teaching_sessions").select("*,classes(class_name,subject_name,color)").order("created_at",{ascending:false});if(error)throw error;return data||[]},[sb])}
...
function SetupScreen({onReady}){
  ...
  return(
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"linear-gradient(135deg,#1E1B4B 0%,#312E81 50%,#1E3A5F 100%)",fontFamily:"'Prompt','Sarabun',sans-serif"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Prompt:wght@400;600;700&display=swap');@keyframes mcSpin{to{transform:rotate(360deg)}}*{box-sizing:border-box}`}</style>
      <div style={{background:"#fff",borderRadius:24,padding:"36px 40px",width:460,maxWidth:"calc(100vw - 32px)",boxShadow:"0 32px 80px rgba(0,0,0,0.3)"}}>
        ...
        <div style={{background:"#F0F9FF",borderRadius:12,padding:"12px 16px",marginBottom:22,border:"1px solid #BAE6FD"}}>
          <div style={{fontSize:13,color:"#0369A1",lineHeight:1.7}}><b>วิธีรับค่า:</b> ไปที่ <b>supabase.com</b> → โปรเจกต์ → <b>Settings → API</b><br/>คัดลอก Project URL และ anon public key</div>
        </div>
        <div style={ss.fRow}><label style={ss.label}>Supabase Project URL</label><input value={url} onChange={e=>setUrl(e.target.value)} style={ss.input} placeholder="https://xxxx.supabase.co" type="url"/></div>
        <div style={ss.fRow}><label style={ss.label}>Anon Public Key</label><input value={key} onChange={e=>setKey(e.target.value)} style={{...ss.input,fontFamily:"monospace",fontSize:12}} placeholder="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."/></div>
        {err&&<div style={{background:T.redL,borderRadius:10,padding:"10px 14px",fontSize:13,color:T.red,marginBottom:16,display:"flex",gap:8,alignItems:"flex-start"}}><AlertCircle size={15} style={{flexShrink:0,marginTop:1}}/>{err}</div>}
        ...
        <div style={{marginTop:20,padding:"14px",background:"#F8FAFC",borderRadius:12,fontSize:12.5,color:"#64748B",lineHeight:1.8}}>
          <b>ขั้นตอนตั้งค่า Supabase ครั้งแรก:</b><br/>
          1. สมัครฟรีที่ supabase.com → สร้าง project ใหม่<br/>
          2. SQL Editor → วางเนื้อหาจาก <code style={{background:"#E2E8F0",padding:"1px 5px",borderRadius:4}}>supabase/schema.sql</code> แล้วกด Run<br/>
          3. Settings → API → คัดลอก URL + anon key มากรอกด้านบน
        </div>
      </div>
    </div>
  )
...
function ModalShell({title,sub,onClose,footer,children,wide}){return <div style={ss.overlay} onClick={onClose}><div style={{...ss.modal,width:wide||480}} onClick={e=>e.stopPropagation()}><div style={ss.mHead}><div><div style={{fontSize:15,fontWeight:700,color:T.text}}>{title}</div>{sub&&<div style={{fontSize:12,color:T.muted}}>{sub}</div>}</div><button onClick={onClose} style={{border:"none",background:"none",cursor:"pointer",padding:4,borderRadius:8}}><X size={18} color={T.muted}/></button></div><div style={ss.mBody}>{children}</div>{footer&&<div style={{padding:"14px 20px",borderTop:`1px solid ${T.border}`,display:"flex",gap:10}}>{footer}</div>}</div></div>}
...
function AddClassModal({sb,onClose,onDone,toast}){
  ...
  const save=async()=>{
    if(!form.class_name||!form.subject_name){toast.err("กรุณากรอกข้อมูลให้ครบ");return}
    setSaving(true)
    try{const{error}=await sb.from("classes").insert(form);if(error)throw new Error(error.message||JSON.stringify(error));toast.ok(`เพิ่มชั้นเรียน ${form.class_name} แล้ว`);onDone();onClose()}catch(e){toast.err(e.message)}finally{setSaving(false)}
  }
...
function AddStudentModal({sb,onClose,onDone,toast}){
  ...
  const f=(k,v)=>setForm(p=>({...p,[k]:v}))
  const save=async()=>{
    if(!form.full_name){toast.err("กรุณากรอกชื่อ");return}
    setSaving(true)
    try{const{error}=await sb.from("students").insert({...form,student_no:form.student_no?parseInt(form.student_no):null});if(error)throw new Error(error.message||JSON.stringify(error));toast.ok(`เพิ่ม ${form.full_name} แล้ว`);onDone();onClose()}catch(e){toast.err(e.message)}finally{setSaving(false)}
  }
  return <ModalShell title="เพิ่มนักเรียน" onClose={onClose} footer={<><button onClick={onClose} style={{...ss.btnO,flex:1,justifyContent:"center"}}>ยกเลิก</button><button onClick={save} disabled={saving} style={{...ss.btnP(),flex:2,justifyContent:"center",opacity:saving?0.7:1}}>{saving?<><Spin size={15} color="#fff"/>กำลังบันทึก...</>:<><Plus size={15}/>เพิ่มนักเรียน</>}</button></>}>
    <div style={{display:"grid",gridTemplateColumns:"80px 1fr",gap:12}}>
      <div style={ss.fRow}><label style={ss.label}>เลขที่</label><input type="number" value={form.student_no} onChange={e=>f("student_no",e.target.value)} style={ss.input} placeholder="1"/></div>
...
function PasteModal({sb,onClose,onDone,toast}){
  ...
  const[saving,setSaving]=useState(false)
  const parsed=text.trim()?text.trim().split("\n").map((line,i)=>{try{const p=line.trim().split(/\t|,/);const isNum=p[0]&&!isNaN(p[0].trim())&&p[0].trim()!=="";return{student_no:isNum?parseInt(p[0]):i+1,full_name:(isNum?(p[1]||""):(p[0]||"")).trim(),level:(isNum?(p[2]||""):(p[1]||"")).trim(),room:(isNum?(p[3]||""):(p[2]||"")).trim()}}catch{return{student_no:i+1,full_name:"",level:"",room:""}}}).filter(s=>s.full_name&&s.full_name.trim()):[]
  const save=async()=>{
  ...
  return <ModalShell title="วางรายชื่อนักเรียน" onClose={onClose} footer={<><button onClick={onClose} style={{...ss.btnO,flex:1,justifyContent:"center"}}>ยกเลิก</button><button onClick={save} disabled={!parsed.length||saving} style={{...ss.btnP(),flex:2,justifyContent:"center",opacity:(!parsed.length||saving)?0.5:1}}>{saving?<><Spin size={15} color="#fff"/>กำลังนำเข้า...</>:<><Upload size={15}/>นำเข้า {parsed.length} คน</>}</button></>}>
    <div style={{background:T.yellowL,borderRadius:10,padding:"10px 14px",marginBottom:14,fontSize:12.5,color:"#78350F"}}>รูปแบบ: <b>เลขที่ [Tab] ชื่อ-นามสกุล [Tab] ชั้น [Tab] ห้อง</b> (แต่ละบรรทัด = 1 คน)</div>
    <textarea value={text} onChange={e=>setText(e.target.value)} rows={8} placeholder={"1\tเด็กหญิงกมลชนก วิชัย\tม.3\t3/1\n2\tเด็กชายขวัญฤดี มีสุข\tม.3\t3/1"} style={{...ss.textarea,fontFamily:"monospace",fontSize:12}}/>
...
function ImportExcelModal({sb,onClose,onDone,toast}){
  ...
  const[saving,setSaving]=useState(false)
  const ref=useRef()
  const parse=file=>{const reader=new FileReader();reader.onload=e=>{try{const wb=XLSX.read(e.target.result,{type:"binary"});const ws=wb.Sheets[wb.SheetNames[0]];const rows=XLSX.utils.sheet_to_json(ws,{header:1,defval:""});setPreview(rows.slice(1).filter(r=>r[1]).map(r=>({student_no:parseInt(r[0])||null,full_name:String(r[1]||"").trim(),level:String(r[2]||"").trim(),room:String(r[3]||"").trim()})))}catch{toast.err("อ่านไฟล์ไม่ได้")}};reader.readAsBinaryString(file)}
  const doImport=async()=>{setSaving(true);try{const{error}=await sb.from("students").insert(preview);if(error)throw new Error(error.message||JSON.stringify(error));toast.ok(`นำเข้าสำเร็จ ${preview.length} คน`);onDone();onClose()}catch(e){toast.err(e.message)}finally{setSaving(false)}}
  return <ModalShell title="นำเข้าจาก Excel" onClose={onClose} footer={<><button onClick={onClose} style={{...ss.btnO,flex:1,justifyContent:"center"}}>ยกเลิก</button><button onClick={doImport} disabled={!preview.length||saving} style={{...ss.btnP(T.green),flex:2,justifyContent:"center",opacity:(!preview.length||saving)?0.5:1}}>{saving?<><Spin size={15} color="#fff"/>กำลังนำเข้า...</>:<><Upload size={15}/>นำเข้า {preview.length} คน</>}</button></>}>
    <div onDragEnter={()=>setDragging(true)} onDragLeave={()=>setDragging(false)} onDrop={e=>{e.preventDefault();setDragging(false);parse(e.dataTransfer.files[0])}} onDragOver={e=>e.preventDefault()} onClick={()=>ref.current?.click()} style={{border:`2px dashed ${dragging?T.purple:"#CBD5E1"}`,borderRadius:14,padding:"32px 20px",textAlign:"center",marginBottom:14,background:dragging?T.purpleL:"#F8FAFC",cursor:"pointer",transition:"all 0.15s"}}>
      <FileSpreadsheet size={40} color={T.green} style={{margin:"0 auto 10px"}}/><div style={{fontSize:14,fontWeight:600,color:"#334155"}}>ลากไฟล์ Excel มาวาง</div><div style={{fontSize:12.5,color:T.muted,marginTop:4}}>หรือคลิกเพื่อเลือก .xlsx, .xls</div>
      <input ref={ref} type="file" accept=".xlsx,.xls" style={{display:"none"}} onChange={e=>e.target.files[0]&&parse(e.target.files[0])}/>
    </div>
    <div style={{background:"#F8FAFC",borderRadius:10,padding:"10px 14px",fontSize:12.5,color:"#64748B",marginBottom:preview.length?12:0}}><b>รูปแบบ:</b> เลขที่ | ชื่อ-นามสกุล | ชั้น | ห้อง (row แรกเป็น header)</div>
    {preview.length>0&&<div><div style={{fontSize:13,color:T.green,fontWeight:600,marginBottom:6,display:"flex",gap:6,alignItems:"center"}}><CheckCircle size={14}/>พบ {preview.length} คน</div><div style={{maxHeight:160,overflowY:"auto",border:`1px solid ${T.border}`,borderRadius:10}}>{preview.slice(0,8).map((s,i)=><div key={i} style={{padding:"6px 12px",fontSize:12.5,borderBottom:`1px solid #F8FAFC`,color:"#475569"}}>{s.student_no}. {s.full_name} <span style={{color:T.muted}}>({s.level} {s.room})</span></div>)}{preview.length>8&&<div style={{padding:"6px 12px",fontSize:12,color:T.muted}}>...และอีก {preview.length-8} คน</div>}</div></div>}
  </ModalShell>
}
...
function SessionModal({sb,onClose,onDone,classes,toast}){
  ...
  const[f,setF]=useState({teach_date:new Date().toISOString().split("T")[0],topic:"",objective:"",activities:"",highlights:"",problems:"",improvements:"",reflection:""})
  const sf=(k,v)=>setF(p=>({...p,[k]:v}))
  const[saving,setSaving]=useState(false)
  const save=async()=>{
    if(!cls){toast.err("เลือกชั้นเรียนก่อน");return};setSaving(true)
    try{const{error}=await sb.from("teaching_sessions").insert({class_id:cls,...f,status:"saved"});if(error)throw new Error(error.message||JSON.stringify(error));toast.ok("บันทึกการสอนแล้ว");onDone();onClose()}catch(e){toast.err(e.message)}finally{setSaving(false)}
  }
  return <ModalShell title="บันทึกการสอน" onClose={onClose} wide={520} footer={<><button onClick={onClose} style={{...ss.btnO,flex:1,justifyContent:"center"}}>ยกเลิก</button><button onClick={save} disabled={saving||!cls} style={{...ss.btnP(),flex:2,justifyContent:"center",opacity:(saving||!cls)?0.6:1}}>{saving?<><Spin size={15} color="#fff"/>กำลังบันทึก...</>:<><Save size={15}/>บันทึก</>}</button></>}>
    <div style={{display:"flex",borderBottom:`1px solid ${T.border}`,marginBottom:16}}>
      {[{id:"a",l:"ข้อมูลหลัก"},{id:"b",l:"บันทึกหลังสอน"}].map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:"9px 0",border:"none",background:"none",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:600,color:tab===t.id?T.purple:T.muted,borderBottom:tab===t.id?`2px solid ${T.purple}`:"2px solid transparent"}}>{t.l}</button>)}
    </div>
    {tab==="a"&&<>
      <div style={ss.fRow}><label style={ss.label}>ชั้นเรียน *</label><select value={cls} onChange={e=>setCls(e.target.value)} style={ss.input}><option value="">-- เลือกชั้นเรียน --</option>{classes.map(c=><option key={c.id} value={c.id}>{c.class_name} · {c.subject_name}</option>)}</select></div>
...
function ProfileModal({sb,onClose,profile,onDone,toast,onDisconnect}){
  ...
  return <ModalShell title="ตั้งค่า" onClose={onClose} footer={<><button onClick={onClose} style={{...ss.btnO,flex:1,justifyContent:"center"}}>ยกเลิก</button><button onClick={save} disabled={saving} style={{...ss.btnP(),flex:2,justifyContent:"center",opacity:saving?0.7:1}}>{saving?<><Spin size={15} color="#fff"/>กำลังบันทึก...</>:<><Save size={15}/>บันทึก</>}</button></>}>
    ...
    <div style={ss.fRow}><label style={ss.label}>ชื่อ-นามสกุลครู</label><input value={form.teacher_name} onChange={e=>sf("teacher_name",e.target.value)} style={ss.input} placeholder="ครูจิราภรณ์ รักเด็ก"/></div>
    ...
    <div style={{borderTop:`1px solid ${T.border}`,paddingTop:16}}>
      <div style={{background:"#F8FAFC",borderRadius:10,padding:"10px 14px",marginBottom:10,fontSize:12.5}}>
        <span style={{color:T.muted}}>ฐานข้อมูล: </span><span style={{fontFamily:"monospace",fontSize:12,color:"#475569"}}>{(store.get("mc_url")||"").replace("https://","").slice(0,32)}...</span>
      </div>
...
const MENU=[
  ...
  {id:"settings",icon:Settings,label:"ตั้งค่า",action:true},
  {id:"backup",icon:Archive,label:"สำรองข้อมูล"},
  {id:"reset",icon:RotateCcw,label:"รีเซทระบบ",danger:true},
]
...
function Dashboard({sb,menu,setMenu,search,setSearch,filterLv,setFilterLv,toast,modal,setModal,onDisconnect}){
  ...
  const sessions=sessQ.data||[];const allSess=allSessQ.data||[];const classes=clsQ.data||[]
  const students=stuQ.data||[];const profile=profQ.data
  ...
  return <>
    ...
    <div style={ss.app}>
      <aside style={ss.sidebar}>
        ...
        <nav style={ss.nav}>{MENU.map(m=><div key={m.id} style={ss.navI(menu===m.id,m.danger)} onClick={()=>{m.action?setModal("profile"):setMenu(m.id)}}><m.icon size={17}/>{m.label}</div>)}</nav>
        <button style={ss.addBtn} onClick={()=>setModal("addClass")}><Plus size={16}/>เพิ่มชั้นเรียน</button>
      </aside>
      <div style={ss.main}>
        <div style={ss.topbar}>
          <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:8,height:8,borderRadius:"50%",background:T.green}}/><span style={{fontSize:12.5,color:T.muted,fontWeight:500}}>Supabase Connected</span></div>
        ...
        <div style={ss.content}>
          ...
          {menu==="dashboard"&&<>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr",gap:16,marginBottom:20}}>
              <div style={ss.card}>
                ...
                {!sessions.length?<EmptyState icon="📋" msg="ยังไม่มีบันทึกการสอน"/>:<table style={ss.table}><thead><tr style={{background:"#F8FAFC"}}>{["ชั้น/วิชา","หัวข้อ","วันที่","สถานะ"].map(h=><th key={h} style={ss.th}>{h}</th>)}</tr></thead><tbody>{sessions.map((s,i)=><tr key={i}><td style={ss.td}><div style={{display:"flex",alignItems:"center",gap:7}}><div style={{width:8,height:8,borderRadius:"50%",background:s.classes?.color||T.purple,flexShrink:0}}/><div><div style={{fontWeight:600,fontSize:13}}>{s.classes?.class_name||"—"}</div><div style={{color:T.muted,fontSize:11.5}}>{s.classes?.subject_name}</div></div></div></td><td style={{...ss.td,color:"#64748B",fontSize:13}}>{s.topic||"—"}</td><td style={{...ss.td,fontSize:12,color:T.muted,whiteSpace:"nowrap"}}>{s.teach_date?new Date(s.teach_date).toLocaleDateString("th-TH",{day:"numeric",month:"short"}):"—"}</td><td style={ss.td}>{s.status==="saved"?<span style={ss.pill("#15803D",T.greenL)}><Check size={11}/>สมบูรณ์</span>:<span style={ss.pill("#C2410C",T.orangeL)}><Clock size={11}/>ร่าง</span>}</td></tr>)}</tbody></table>}
              </div>
              <div style={ss.card}><div style={ss.cHead}><span style={ss.cTitle}>สรุปการเข้าเรียน</span>{dashQ.loading&&<Spin size={16}/>}</div>{!dashQ.loading&&<AttChart att={stats.att}/>}</div>
            </div>
            {classes.length>0&&<><div style={{fontSize:14.5,fontWeight:700,color:T.text,marginBottom:12}}>ชั้นเรียนของฉัน</div><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>{classes.slice(0,6).map(c=>{const cnt=c.class_students?.[0]?.count??0;return <div key={c.id} style={{background:"#fff",borderRadius:14,border:`1px solid ${T.border}`,overflow:"hidden",cursor:"pointer"}} onClick={()=>setMenu("classes")}><div style={{height:5,background:c.color||T.purple}}/><div style={{padding:"14px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div><div style={{fontSize:15,fontWeight:700,color:T.text}}>{c.class_name}</div><div style={{fontSize:12.5,color:T.muted}}>{c.subject_name}</div></div><div style={{width:36,height:36,borderRadius:10,background:(c.color||T.purple)+"22",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,color:c.color||T.purple,fontSize:15}}>{c.class_name?.[0]}</div></div><div style={{fontSize:12.5,color:"#64748B"}}><Users size={12} style={{verticalAlign:"middle"}}/> {cnt} คน</div></div></div>})}</div></>}
          </>}
          ...
          {menu==="classes"&&<>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16}}>{classes.map(c=>{const cnt=c.class_students?.[0]?.count??0;return <div key={c.id} style={{background:"#fff",borderRadius:16,border:`1px solid ${T.border}`,overflow:"hidden"}}><div style={{height:6,background:c.color||T.purple}}/><div style={{padding:"16px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div><div style={{fontSize:16,fontWeight:700,color:T.text}}>{c.class_name}</div><div style={{fontSize:13,color:T.muted}}>{c.subject_name}</div></div><div style={{width:40,height:40,borderRadius:12,background:(c.color||T.purple)+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:700,color:c.color||T.purple}}>{c.class_name?.[0]}</div></div><div style={{fontSize:12.5,color:"#64748B",marginBottom:14}}><Users size={12} style={{verticalAlign:"middle"}}/> {cnt} คน</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><button onClick={()=>setModal("session")} style={{padding:"8px 0",background:c.color||T.purple,color:"#fff",border:"none",borderRadius:10,cursor:"pointer",fontSize:12.5,fontWeight:600,fontFamily:"inherit"}}>▶ บันทึกสอน</button><button onClick={async()=>{if(!window.confirm(`ลบ ${c.class_name}?`))return;await sb.from("classes").delete().eq("id",c.id);toast.ok("ลบแล้ว");clsQ.reload();dashQ.reload()}} style={{padding:"8px 0",background:T.redL,color:T.red,border:"none",borderRadius:10,cursor:"pointer",fontSize:12.5,fontWeight:600,fontFamily:"inherit"}}>ลบ</button></div></div></div>})}</div>}
          </>}
          ...
          {menu==="students"&&<>
            ...
            <div style={ss.card}><table style={ss.table}><thead><tr style={{background:"#F8FAFC"}}>{["#","ชื่อ-นามสกุล","ชั้น","ห้อง",""].map((h,i)=><th key={i} style={ss.th}>{h}</th>)}</tr></thead><tbody>{students.map((stu,i)=><tr key={stu.id}><td style={{...ss.td,color:T.muted,fontSize:12.5}}>{stu.student_no||i+1}</td><td style={ss.td}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:30,height:30,borderRadius:"50%",background:T.blueL,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:T.blue,flexShrink:0}}>{stu.full_name?.[0]}</div>{stu.full_name}</div></td><td style={ss.td}>{stu.level}</td><td style={ss.td}>{stu.room}</td><td style={ss.td}><button onClick={async()=>{if(!window.confirm(`ลบ ${stu.full_name}?`))return;await sb.from("students").delete().eq("id",stu.id);toast.ok("ลบแล้ว");stuQ.reload();dashQ.reload()}} style={{padding:"5px 8px",border:`1px solid #FECACA`,borderRadius:8,cursor:"pointer",background:T.redL,color:T.red}}><Trash2 size={13}/></button></td></tr>)}</tbody></table><div style={{padding:"10px 14px",background:"#F8FAFC",fontSize:12.5,color:T.muted,borderTop:`1px solid #F1F5F9`}}>แสดง {students.length} คน</div></div>}
          </>}

          {menu==="session"&&<>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18}}>
              <div><div style={{fontSize:18,fontWeight:700,color:T.text}}>บันทึกการสอน</div><div style={{fontSize:13,color:T.muted}}>บันทึกแผนและผลการจัดการเรียนรู้</div></div>
              <button style={ss.btnP()} onClick={()=>setModal("session")}><Plus size={15}/>สร้างบันทึกใหม่</button>
            </div>
            {allSessQ.loading?<PageLoad/>:!allSess.length?<EmptyState icon="📋" msg="ยังไม่มีบันทึกการสอน" btn={<button style={ss.btnP()} onClick={()=>setModal("session")}><Plus size={15}/>สร้างบันทึกแรก</button>}/>:
            <div style={ss.card}><table style={ss.table}><thead><tr style={{background:"#F8FAFC"}}>{["ชั้น/วิชา","เรื่องที่สอน","วันที่","จุดประสงค์","สถานะ"].map(h=><th key={h} style={ss.th}>{h}</th>)}</tr></thead><tbody>{allSess.map((s,i)=><tr key={i}><td style={ss.td}><div style={{display:"flex",alignItems:"center",gap:7}}><div style={{width:8,height:8,borderRadius:"50%",background:s.classes?.color||T.purple,flexShrink:0}}/><div><div style={{fontWeight:600,fontSize:13}}>{s.classes?.class_name||"—"}</div><div style={{color:T.muted,fontSize:11.5}}>{s.classes?.subject_name}</div></div></div></td><td style={{...ss.td,maxWidth:180}}><div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontSize:13}}>{s.topic||"—"}</div></td><td style={{...ss.td,whiteSpace:"nowrap",color:T.muted,fontSize:12}}>{s.teach_date?new Date(s.teach_date).toLocaleDateString("th-TH",{day:"numeric",month:"short",year:"2-digit"}):"—"}</td><td style={{...ss.td,maxWidth:160}}><div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontSize:12,color:"#64748B"}}>{s.objective||"—"}</div></td><td style={ss.td}>{s.status==="saved"?<span style={ss.pill("#15803D",T.greenL)}><Check size={11}/>สมบูรณ์</span>:<span style={ss.pill("#C2410C",T.orangeL)}><Clock size={11}/>ร่าง</span>}</td></tr>)}</tbody></table></div>}
          </>}

          {!["dashboard","classes","students","session"].includes(menu)&&<div style={{textAlign:"center",paddingTop:60}}><div style={{fontSize:60,marginBottom:16}}>🚧</div><div style={{fontSize:18,fontWeight:600,color:T.muted}}>{MENU.find(m=>m.id===menu)?.label}</div><div style={{fontSize:13,color:"#CBD5E1",marginTop:6}}>อยู่ระหว่างพัฒนา</div></div>}
...
export default function App(){
  ...
  return <Dashboard sb={sb} menu={menu} setMenu={setMenu} search={search} setSearch={setSearch} filterLv={filterLv} setFilterLv={setFilterLv} toast={toast} modal={modal} setModal={setModal} onDisconnect={handleDisconnect}/>
}
