// ═══════════════════════════════════════════════════════════════
//  MyClass — ระบบบันทึกการสอน (พร้อมใช้งาน + Supabase จริง)
//  วางไฟล์นี้ที่: src/App.jsx
//  ไม่ต้องการ import อื่นเพิ่ม — ทุกอย่างอยู่ในไฟล์เดียว
//
//  dependencies: @supabase/supabase-js, recharts, lucide-react, xlsx
//  .env:  VITE_SUPABASE_URL=...  VITE_SUPABASE_ANON_KEY=...
//         (ถ้าไม่มี .env แอปจะแสดงหน้า Setup อัตโนมัติ)
// ═══════════════════════════════════════════════════════════════
...
import {
  ...
  CheckCircle, Database, WifiOff, Search, Save, ChevronDown,
  Star, CalendarDays, Camera, Pencil, ClipboardList, Activity
} from "lucide-react"
...
function getSB(url, key) {
  if (_sb) return _sb
  const u = url || localStorage.getItem("mc_url") || import.meta.env?.VITE_SUPABASE_URL || ""
  const k = key || localStorage.getItem("mc_key") || import.meta.env?.VITE_SUPABASE_ANON_KEY || ""
  if (!u || !k) return null
  _sb = createClient(u, k)
  return _sb
}
...
const ss = {
  ...
  // Nav
  logo:    { display:"flex", alignItems:"center", gap:10, padding:"18px 16px 14px", borderBottom:`1px solid ${T.border}` },
...
function PageLoad({ text="กำลังโหลด..." }) {
  return <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:280, gap:12 }}><Spin size={34}/><span style={{ color:T.muted, fontSize:14 }}>{text}</span></div>
}
function EmptyState({ icon="📭", msg="ไม่มีข้อมูล", btn }) {
  return <div style={{ textAlign:"center", padding:"52px 24px" }}><div style={{ fontSize:48, marginBottom:12 }}>{icon}</div><div style={{ fontSize:15, fontWeight:600, color:T.muted }}>{msg}</div>{btn&&<div style={{ marginTop:16 }}>{btn}</div>}</div>
}
function Toast({ list }) {
  if (!list.length) return null
  return (
    <div style={{ position:"absolute", top:12, right:12, zIndex:200, display:"flex", flexDirection:"column", gap:8 }}>
      {list.map(t=>(
        <div key={t.id} style={{ background:t.type==="err"?T.red:T.green, color:"#fff", borderRadius:12, padding:"11px 16px", fontSize:13.5, fontWeight:600, display:"flex", alignItems:"center", gap:8, fontFamily:"inherit", boxShadow:"0 4px 20px rgba(0,0,0,0.18)", animation:"mcSlide 0.2s ease", maxWidth:310 }}>
...
function useClasses(sb) {
  return useQ(async () => {
    const { data, error } = await sb.from("classes")
      .select("*, class_students(count)").order("created_at", { ascending:false })
    if (error) throw error; return data||[]
  }, [sb])
}
...
function useStudents(sb, search, level) {
  return useQ(async () => {
    ...
    if (level !== "ทั้งหมด") q = q.eq("level", level)
...
function SetupScreen({ onReady }) {
  const [url, setUrl] = useState(localStorage.getItem("mc_url")||"")
  const [key, setKey] = useState(localStorage.getItem("mc_key")||"")
  ...
  const connect = async () => {
    ...
    try {
      ...
      localStorage.setItem("mc_url", url.trim())
      localStorage.setItem("mc_key", key.trim())
      resetSB(); onReady(url.trim(), key.trim())
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", minHeight:"100vh", background:`linear-gradient(135deg,#1E1B4B 0%,#312E81 50%,#1E3A5F 100%)`, fontFamily:"'Prompt','Sarabun',sans-serif" }}>
      <div style={{ background:"#fff", borderRadius:24, padding:"36px 40px", width:460, boxShadow:"0 32px 80px rgba(0,0,0,0.3)" }}>
        {/* Header */}
        ...
        <div style={{ background:"#F0F9FF", borderRadius:12, padding:"12px 16px", marginBottom:22, border:"1px solid #BAE6FD" }}>
          <div style={{ fontSize:13, color:"#0369A1", lineHeight:1.7 }}>
            <b>วิธีรับค่า:</b> ไปที่ <b>supabase.com</b> → โปรเจกต์ของคุณ<br/>
        ...
        <div style={ss.fRow}>
          ...
          <input value={url} onChange={e=>setUrl(e.target.value)} style={ss.input} placeholder="https://xxxx.supabase.co" type="url"/>
        </div>
        <div style={ss.fRow}>
          <label style={ss.label}>Anon Public Key</label>
          <input value={key} onChange={e=>setKey(e.target.value)} style={{ ...ss.input, fontFamily:"monospace", fontSize:12 }} placeholder="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."/>
        </div>
        ...
        <div style={{ marginTop:20, padding:"14px", background:"#F8FAFC", borderRadius:12, fontSize:12.5, color:"#64748B", lineHeight:1.8 }}>
          <b>ยังไม่มี Supabase?</b><br/>
          1. สมัครฟรีที่ supabase.com<br/>
          2. สร้างโปรเจกต์ใหม่<br/>
          3. รัน SQL จากไฟล์ <code style={{ background:"#E2E8F0", padding:"1px 5px", borderRadius:4 }}>supabase/schema.sql</code><br/>
          4. กลับมากรอก URL และ Key ด้านบน
        </div>
      </div>
    </div>
  )
    ...
    <div style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 18px 18px" }}>
      <div style={{ width:160, height:160, flexShrink:0 }}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart><Pie data={items} cx="50%" cy="50%" innerRadius={42} outerRadius={70} dataKey="value" labelLine={false} label={PIE_LABEL}>
            {items.map((d,i)=><Cell key={i} fill={d.color}/>)}
          </Pie><Tooltip contentStyle={{ fontFamily:"Prompt,sans-serif", fontSize:13, borderRadius:10 }}/></PieChart>
        </ResponsiveContainer>
      </div>
  ...
  return (
    <div style={ss.overlay} onClick={onClose}>
      <div style={{ ...ss.modal, width:wide||480 }} onClick={e=>e.stopPropagation()}>
        <div style={ss.mHead}>
          <div><div style={{ fontSize:15, fontWeight:700, color:T.text }}>{title}</div>{sub&&<div style={{ fontSize:12, color:T.muted }}>{sub}</div>}</div>
        ...
        {footer && <div style={{ padding:"14px 20px", borderTop:`1px solid ${T.border}`, display:"flex", gap:10 }}>{footer}</div>}
      </div>
    </div>
  )
}
...
function AddClassModal({ sb, onClose, onDone, toast }) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      const { error } = await sb.from("classes").insert(form)
  ...
  return (
    <ModalShell title="เพิ่มชั้นเรียน" onClose={onClose}
      footer={<>
        ...
        <button onClick={save} disabled={saving} style={{ ...ss.btnP(), flex:2, justifyContent:"center", opacity:saving?.7:1 }}>
          {saving?<><Spin size={15} color="#fff"/>กำลังบันทึก...</>:<><Check size={15}/>เพิ่มชั้นเรียน</>}
        </button>
      </>}>
      <div style={ss.fRow}><label style={ss.label}>ชื่อห้อง *</label><input value={form.class_name} onChange={e=>f("class_name",e.target.value)} style={ss.input} placeholder="ม.3/1"/></div>
...
function AddStudentModal({ sb, onClose, onDone, toast }) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      const { error } = await sb.from("students").insert({ ...form, student_no:form.student_no?parseInt(form.student_no):null })
      if (error) throw error
...
function PasteModal({ sb, onClose, onDone, toast }) {
  ...
  const parsed = text.trim() ? text.trim().split("\n").map((line,i)=>{
    ...
    const isNum = !isNaN(p[0])
    return {
      student_no: isNum ? parseInt(p[0]) : i+1,
  ...
  return (
    <ModalShell title="วางรายชื่อนักเรียน" onClose={onClose}
      footer={<>
        <button onClick={onClose} style={{ ...ss.btnO, flex:1, justifyContent:"center" }}>ยกเลิก</button>
        <button onClick={save} disabled={!parsed.length||saving} style={{ ...ss.btnP(), flex:2, justifyContent:"center", opacity:(!parsed.length||saving)?.5:1 }}>
          {saving?<><Spin size={15} color="#fff"/>กำลังนำเข้า...</>:<><Upload size={15}/>นำเข้า {parsed.length} คน</>}
        </button>
      </>}>
      <div style={{ background:T.yellowL, borderRadius:10, padding:"10px 14px", marginBottom:14, fontSize:12.5, color:"#78350F" }}>
        รูปแบบ: <b>เลขที่ [Tab] ชื่อ-นามสกุล [Tab] ชั้น [Tab] ห้อง</b> (แต่ละบรรทัด = นักเรียน 1 คน)
      </div>
...
function ImportExcelModal({ sb, onClose, onDone, toast }) {
  ...
  return (
    <ModalShell title="นำเข้าจาก Excel" onClose={onClose}
      footer={<>
        <button onClick={onClose} style={{ ...ss.btnO, flex:1, justifyContent:"center" }}>ยกเลิก</button>
        <button onClick={doImport} disabled={!preview.length||saving} style={{ ...ss.btnP(T.green), flex:2, justifyContent:"center", opacity:(!preview.length||saving)?.5:1 }}>
      ...
      <div onDragEnter={()=>setDragging(true)} onDragLeave={()=>setDragging(false)}
        onDrop={e=>{e.preventDefault();setDragging(false);parse(e.dataTransfer.files[0])}} onDragOver={e=>e.preventDefault()}
      ...
      <div style={{ background:"#F8FAFC", borderRadius:10, padding:"10px 14px", fontSize:12.5, color:"#64748B", marginBottom:preview.length?12:0 }}>
        <b>รูปแบบ:</b> เลขที่ | ชื่อ-นามสกุล | ชั้น | ห้อง (row แรกเป็น header)
      </div>
      {preview.length > 0 && (
        <div>
          <div style={{ fontSize:13, color:T.green, fontWeight:600, marginBottom:6, display:"flex", gap:6, alignItems:"center" }}><CheckCircle size={14}/>พบ {preview.length} คน</div>
          <div style={{ maxHeight:160, overflowY:"auto", border:`1px solid ${T.border}`, borderRadius:10 }}>
            {preview.slice(0,8).map((s,i)=><div key={i} style={{ padding:"6px 12px", fontSize:12.5, borderBottom:`1px solid #F8FAFC`, color:"#475569" }}>{s.student_no}. {s.full_name} <span style={{ color:T.muted }}>({s.level}/{s.room})</span></div>)}
...
function SessionModal({ sb, onClose, onDone, classes, toast }) {
  ...
  const tabs = [{ id:"a", l:"ข้อมูลหลัก" },{ id:"b", l:"บันทึกหลังสอน" }]
  ...
  return (
    <ModalShell title="บันทึกการสอน" sub="ม.3/1 · คณิตศาสตร์" onClose={onClose} wide={520}
      footer={<>
        <button onClick={onClose} style={{ ...ss.btnO, flex:1, justifyContent:"center" }}>ยกเลิก</button>
        <button onClick={save} disabled={saving||!cls} style={{ ...ss.btnP(), flex:2, justifyContent:"center", opacity:(saving||!cls)?.6:1 }}>
      ...
      <div style={{ display:"flex", borderBottom:`1px solid ${T.border}`, marginBottom:16 }}>
        {tabs.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{ flex:1, padding:"9px 0", border:"none", background:"none", cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:600, color:tab===t.id?T.purple:T.muted, borderBottom:tab===t.id?`2px solid ${T.purple}`:"2px solid transparent" }}>{t.l}</button>)}
      </div>
      {tab==="a" && <>
        <div style={ss.fRow}>
    ...
    <ModalShell title="ตั้งค่า" onClose={onClose}
      ...
      <div style={{ borderTop:`1px solid ${T.border}`, paddingTop:16 }}>
        <div style={{ background:"#F8FAFC", borderRadius:10, padding:"10px 14px", marginBottom:10, fontSize:12.5 }}>
          ...
          <span style={{ fontFamily:"monospace", fontSize:12, color:"#475569" }}>{localStorage.getItem("mc_url")?.replace("https://","").slice(0,30)}...</span>
        </div>
...
export default function App() {
  ...
  // Modals
  const [modal, setModal] = useState(null) // "addClass" | "addStu" | "paste" | "excel" | "session" | "profile"

  // On mount, try to init from stored creds
  useEffect(() => {
    const u = localStorage.getItem("mc_url")||import.meta.env?.VITE_SUPABASE_URL||""
    const k = localStorage.getItem("mc_key")||import.meta.env?.VITE_SUPABASE_ANON_KEY||""
    if (u && k) {
      const client = createClient(u, k)
      setSb(client); setConnected(true); setSbConfig({ url:u, key:k })
    }
  }, [])

  const handleReady = (url, key) => {
    const client = createClient(url, key)
    setSb(client); setSbConfig({ url, key }); setConnected(true)
  }

  const handleDisconnect = () => {
    localStorage.removeItem("mc_url"); localStorage.removeItem("mc_key")
    resetSB(); setSb(null); setConnected(false)
  }
...
function Dashboard({ sb, menu, setMenu, search, setSearch, filterLv, setFilterLv, toast, modal, setModal, onDisconnect }) {
  ...
  const reloadAll = () => { dashQ.reload(); sessQ.reload(); clsQ.reload(); stuQ.reload(); profQ.reload() }

  const thaiDate = () => new Date().toLocaleDateString("th-TH",{ year:"numeric", month:"long", day:"numeric", weekday:"long" })

  return (
    <>
      <style>{`@keyframes mcSpin{to{transform:rotate(360deg)}} @keyframes mcSlide{from{transform:translateX(24px);opacity:0}to{transform:translateX(0);opacity:1}} *{box-sizing:border-box}`}</style>
      ...
      <div style={ss.app}>
        ...
        <aside style={ss.sidebar}>
          ...
          <nav style={ss.nav}>
            {MENU.map(m=>(
              <div key={m.id} style={ss.navI(menu===m.id, m.danger)}
                onClick={()=>{ m.action ? setModal("profile") : setMenu(m.id) }}>
        ...
        <div style={ss.main}>
          ...
          <div style={ss.topbar}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <button onClick={reloadAll} style={{ border:"none", background:"none", cursor:"pointer", padding:6, borderRadius:8 }}>
          ...
          <div style={ss.content}>
            ...
            {menu==="dashboard" && <>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1.4fr 1fr", gap:16, marginBottom:20 }}>
                ...
                <div style={ss.card}>
                  ...
                  {!sessions.length ? <EmptyState icon="📋" msg="ยังไม่มีบันทึกการสอน"/> : (
                    <table style={ss.table}>
                      ...
                      <tbody>{sessions.map((s,i)=>(
                        <tr key={i}>
                          ...
                          <td style={{ ...ss.td, color:"#64748B", fontSize:13 }}>{s.topic||"—"}</td>
              ...
              {classes.length>0 && <>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14 }}>
                  {classes.slice(0,6).map(c=>{
                    const cnt = c.class_students?.[0]?.count??0
                    return (
                      <div key={c.id} style={{ background:"#fff", borderRadius:14, border:`1px solid ${T.border}`, overflow:"hidden", cursor:"pointer" }} onClick={()=>setMenu("classes")}>
                        ...
                        <div style={{ padding:"14px" }}>
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                            <div><div style={{ fontSize:15, fontWeight:700, color:T.text }}>{c.class_name}</div><div style={{ fontSize:12.5, color:T.muted }}>{c.subject_name}</div></div>
                          ...
                          <div style={{ fontSize:12.5, color:"#64748B" }}><Users size={12} style={{ verticalAlign:"middle" }}/> {cnt} คน</div>
                        </div>
                      </div>
                    )
                  })}
            ...
            {menu==="classes" && <>
              ...
              {clsQ.loading ? <PageLoad/> : !classes.length ? (
                    ...
                    return (
                      <div key={c.id} style={{ background:"#fff", borderRadius:16, border:`1px solid ${T.border}`, overflow:"hidden" }}>
                        ...
                        <div style={{ padding:"16px" }}>
                          ...
                          <div style={{ fontSize:12.5, color:"#64748B", marginBottom:14 }}><Users size={12} style={{ verticalAlign:"middle" }}/> {cnt} คน</div>
            ...
            {menu==="students" && <>
              ...
              {stuQ.loading ? <PageLoad/> : !students.length ? (
                ...
                <div style={ss.card}>
                  <table style={ss.table}>
                    ...
                    <tbody>{students.map((stu,i)=>(
                      <tr key={stu.id}>
                        <td style={{ ...ss.td, color:T.muted, fontSize:12.5 }}>{stu.student_no||i+1}</td>
                        <td style={ss.td}>
                          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                            <div style={{ width:30, height:30, borderRadius:"50%", background:T.blueL, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:700, color:T.blue, flexShrink:0 }}>{stu.full_name?.[0]}</div>
                            {stu.full_name}
                          </div>
                        </td>
                        ...
                        <td style={ss.td}>
                          <button onClick={async()=>{
                            ...
                            await sb.from("students").delete().eq("id",stu.id)
            ...
            {menu==="session" && <>
              ...
              {sessQ.loading ? <PageLoad/> : !sessions.length ? (
                  ...
                  <table style={ss.table}>
                    ...
                    <tbody>{sessions.map((s,i)=>(
                      <tr key={i}>
                        ...
                        <td style={{ ...ss.td, maxWidth:180 }}><div style={{ overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", fontSize:13 }}>{s.topic||"—"}</div></td>
            ...
            {/* ── OTHER PAGES ───────────────────────── */}
            {!["dashboard","classes","students","session"].includes(menu) && (
              <div style={{ textAlign:"center", paddingTop:60 }}>
                <div style={{ fontSize:60, marginBottom:16 }}>🚧</div>
                <div style={{ fontSize:18, fontWeight:600, color:T.muted }}>{MENU.find(m=>m.id===menu)?.label}</div>
                <div style={{ fontSize:13, color:"#CBD5E1", marginTop:6 }}>อยู่ระหว่างพัฒนา</div>
              </div>
            )}
        ...
        {modal==="session"   && <SessionModal     sb={sb} onClose={()=>setModal(null)} onDone={()=>{sessQ.reload();dashQ.reload()}} classes={classes} toast={toast}/>}
